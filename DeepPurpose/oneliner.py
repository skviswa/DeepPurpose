...
def repurpose(target, target_name=None,
	...
	pretrained_model_names = [['MPNN', 'CNN'], ['CNN', 'CNN'], ['Morgan', 'CNN'], ['Morgan', 'AAC'],
							  ['Daylight', 'AAC']]
	...
	if train_drug is None:
		...
		for idx, model_name in enumerate(pretrained_model_names):
			...
			y_pred = models.repurpose(X_repurpose, target, model, drug_names, target_name, convert_y=convert_y,
									  result_folder=result_folder_path, verbose=False)
			y_preds_models.append(y_pred)
			print('Predictions from model ' + str(idx + 1) + ' with drug encoding ' + model_name[
				0] + ' and target encoding ' + model_name[1] + ' are done...')
			print('-------------')
	else:
		...
		for idx, model_name in enumerate(pretrained_model_names):
			...
			y_pred = models.repurpose(X_repurpose, target, model, drug_names, target_name, convert_y=convert_y,
									  result_folder=result_folder_path, verbose=False)
			y_preds_models.append(y_pred)
			print('Predictions from model ' + str(idx) + ' with drug encoding ' + model_name[
				0] + ' and target encoding ' + model_name[1] + ' are done...')
...
def virtual_screening(target, X_repurpose=None,
	...
	if train_drug is None:
		...
		for idx, model_name in enumerate(pretrained_model_names):
			...
			y_pred = models.virtual_screening(X_repurpose, target, model, drug_names, target_name, convert_y=convert_y,
											  result_folder=result_folder_path, verbose=False)
			y_preds_models.append(y_pred)
			print('Predictions from model ' + str(idx + 1) + ' with drug encoding ' + model_name[
				0] + ' and target encoding ' + model_name[1] + ' are done...')
