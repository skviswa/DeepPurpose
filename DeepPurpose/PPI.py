...
class PPI_Model:
    ...
    def train(self, train, val, test=None, verbose=True):
        ...
        training_generator = data.DataLoader(
            data_process_PPI_loader(train.index.values, train.Label.values, train, **self.config), **params)
        validation_generator = data.DataLoader(
            data_process_PPI_loader(val.index.values, val.Label.values, val, **self.config), **params)
        ...
        if test is not None:
            ...
            testing_generator = data.DataLoader(
                data_process_PPI_loader(test.index.values, test.Label.values, test, **self.config), **params_test)
                ...
                if self.binary:
                    ...
                    if verbose:
                        print('Validation at Epoch ' + str(epo + 1) + ' , MSE: ' + str(mse)[
                              ...
                              + str(r2)[:7] + ' with p-value: ' + str(p_val)[:7] + ' , Concordance Index: ' + str(CI)[
                                                                                                              :7])
            table.add_row(lst)
